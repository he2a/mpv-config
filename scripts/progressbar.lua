...
settings['default-style'] = [[\fnGeogrotesque-SemiBold\b0\bord2\shad0\fs30\c&HFFFFFF&\3c&H2D2D2D&]]
...
settings['bar-foreground-style'] = [[\c&H0000D5&]]
...
settings['hover-time-left-margin'] = 150
...
settings['hover-time-right-margin'] = 150
...
settings['pause-indicator-foreground-style'] = [[\c&H9E9E9E&]]
...
mp.add_forced_key_binding("mouse_btn0", "left-click", function()
...
do
  ...
  _class_0 = setmetatable({
    __init = function(self)
      ...
      mp.add_forced_key_binding("tab", "request-display", function(event)
      ...
      return mp.add_forced_key_binding('ctrl+r', 'reconfigure', (function()
        local _base_1 = self
    ...
    redraw = function(self)
      if self.active then
        ...
        if timeElapsed ~= self.lastTime then
          local update = true
          self.line[4] = ('%02d:%02d:%02d'):format(math.floor(timeElapsed / 3600), math.floor((timeElapsed / 60) % 60), math.floor(timeElapsed % 60))
    ...
    redraw = function(self)
      if self.active then
        ...
        if timeRemaining ~= self.lastTime then
          local update = true
          self.line[4] = ('%02d:%02d:%02d'):format(math.floor(timeRemaining / 3600), math.floor((timeRemaining / 60) % 60), math.floor(timeRemaining % 60))
      ...
      if self.active then
        ...
        if Mouse.x ~= self.lastX then
          ...
          if hoverTime ~= self.lastTime then
            self.line[4] = ([[%02d:%02d:%02d]]):format(math.floor(hoverTime / 3600), math.floor((hoverTime / 60) % 60), math.floor(hoverTime % 60))
...
if settings['enable-bar'] then
  ...
  mp.add_forced_key_binding("C", "toggle-inactive-bar", function()
...
if settings['pause-indicator'] then
  ...
  mp.add_forced_key_binding('.', 'step-forward', function()
    notFrameStepping = false
    return mp.commandv('frame_step')
  end, {
    repeatable = true
  })
  mp.add_forced_key_binding(',', 'step-backward', function()
    notFrameStepping = false
